...
def looks_cultural(r):
    parts = [r.get("title"), r.get("description"), r.get("keywords"), r.get("theme"), r.get("themes"), r.get("origin_agenda_title")]
...
fields = ["uid", "title", "firstdate_begin", "firstdate_end", "lastdate_begin", "lastdate_end", "date_first", "date_last", "keywords", "theme", "themes", "origin_agenda_title"]
print("--- 10 Records ---")
...
maybe = [r for r in recs if not looks_cultural(r) and any(w in (to_text(r.get("origin_agenda_title"))+to_text(r.get("theme"))+to_text(r.get("themes"))).lower() for w in ["loisir", "animation", "fête", "fete", "sport", "social", "jeunesse", "jeune"])]
for r in maybe[:5]: print(f"UID: {r.get('uid')} | Title: {r.get('title')} | Agenda: {r.get('origin_agenda_title')} | Theme: {r.get('theme')}")
